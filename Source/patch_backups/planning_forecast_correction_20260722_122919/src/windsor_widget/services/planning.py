...
def _commitment_position(
    ...
    *,
    ...
    return CommitmentPosition(
        ...
        # Retained for backwards compatibility with the first read model.  Cover is now
        # reference-only, so there is deliberately no calculated alignment balance.
        cover_inbound_balance=_ZERO,
        projected_pool=projected_pool,
        immediate_shortage=max(_ZERO, -physical_pool),
        uncovered_cover=_ZERO,
    )
    ...
    if inventory is None or commitments is None:
        ...
        if trend.significant and trend.delta > 0:
            ...
            reasons.append(
                ...
                f"({percent_text}); the trend changes the target before supply is netted."
            )
...
def get_order_analysis(
    ...
    *,
    ...
    for item, inventory_line in inventory_rows:
        ...
        if trend.significant and trend.delta > 0:
            reason_parts.append(
                f"{trend_mode} invoiced demand increased by {trend.delta}; "
                "trend applied to target before supply"
            )
        if commitments.immediate_shortage > 0:
