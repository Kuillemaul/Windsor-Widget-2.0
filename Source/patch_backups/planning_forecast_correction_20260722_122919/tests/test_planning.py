...
def _seed(session: Session) -> None:
    ...
    sales_batch, sales_rows = _batch(session, "sales_transactions", 8, "sales")
    ...
    recent_order = SalesDocument(
        customer_account_id=customer.customer_account_id,
        myob_customer_record_id="C1",
        invoice_no="ORD-RECENT",
        first_transaction_date=date(2026, 6, 10),
        last_transaction_date=date(2026, 6, 10),
        line_count=1,
        first_import_batch_id=sales_batch.import_batch_id,
        last_import_batch_id=sales_batch.import_batch_id,
    )
    stale_order = SalesDocument(
        customer_account_id=customer.customer_account_id,
        myob_customer_record_id="C1",
        invoice_no="ORD-STALE",
        first_transaction_date=date(2026, 3, 1),
        last_transaction_date=date(2026, 3, 1),
        line_count=1,
        first_import_batch_id=sales_batch.import_batch_id,
        last_import_batch_id=sales_batch.import_batch_id,
    )
    session.add_all([recent_order, stale_order])
    session.flush()
    session.add_all(
        [
            SalesLine(
                sales_document_id=recent_order.sales_document_id,
                item_id=item.item_id,
                line_sequence=1,
                source_import_row_id=sales_rows[6].import_row_id,
                source_row_sha256=sales_rows[6].row_sha256,
                last_import_batch_id=sales_batch.import_batch_id,
                myob_item_number="I1",
                customer_name_snapshot="Customer A",
            ...
            SalesLine(
                sales_document_id=stale_order.sales_document_id,
                item_id=item.item_id,
                line_sequence=1,
                source_import_row_id=sales_rows[7].import_row_id,
                source_row_sha256=sales_rows[7].row_sha256,
                last_import_batch_id=sales_batch.import_batch_id,
                myob_item_number="I1",
                customer_name_snapshot="Customer A",
...
def test_item_planning_separates_sales_demand_cover_alignment_and_stale_commitments() -> None:
    with Session(_engine()) as session:
        ...
        assert analysis.inventory.available == Decimal("5")
        ...
        assert analysis.commitments.cover_inbound_balance == Decimal("0")
        ...
        assert analysis.adjusted_suggested_order == Decimal("40")
        assert analysis.status == "order"
        assert any("Ignored 12" in reason for reason in analysis.reasons)
        assert not any("Cover alignment" in reason for reason in analysis.reasons)
        ...
        # Customer cover is a standing coverage reference, not additional demand.
        cover_line = session.scalar(select(CoverOrderLine))
        cover_line.quantity = Decimal("200")
        session.commit()
        changed_cover = get_item_planning_analysis(
            session,
            "I1",
            analysis_months=6,
            fallback_lead_weeks=14,
        ...
        assert changed_cover.commitments.cover_inbound_balance == Decimal("0")
        ...
        assert changed_cover.adjusted_suggested_order == Decimal("40")
        ...
        inventory_line.on_order = Decimal("100")
        inventory_line.available = Decimal("83")
        session.commit()
        with_inbound = get_item_planning_analysis(
            session,
            "I1",
            analysis_months=6,
            fallback_lead_weeks=14,
...
def test_order_analysis_and_readiness_are_ui_ready() -> None:
    with Session(_engine()) as session:
        ...
        assert result.rows[0].cover_inbound_balance == Decimal("0")
        assert result.rows[0].projected_pool == Decimal("42")
        assert result.rows[0].adjusted_target_stock > result.rows[0].target_stock
        assert result.rows[0].adjusted_suggested_order == Decimal("40")
        assert "cover alignment" not in result.rows[0].reason.lower()
...
def test_physical_shortage_is_critical_even_when_inbound_exists() -> None:
    with Session(_engine()) as session:
        _seed(session)
        item = session.scalar(select(Item).where(Item.item_number == "I1"))
        recent_document = session.scalar(
            select(SalesDocument).where(SalesDocument.invoice_no == "ORD-RECENT")
        )
        recent_line = session.scalar(
            select(SalesLine).where(
                SalesLine.sales_document_id == recent_document.sales_document_id
            )
        )
        recent_line.quantity = Decimal("50")
        session.commit()
