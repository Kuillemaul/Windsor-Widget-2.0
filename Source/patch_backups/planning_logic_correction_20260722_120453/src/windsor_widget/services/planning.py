...
Planning separates demand, commitments, inbound supply and cover alignment:

* invoiced sales are the only demand-history signal
* physical pool = On Hand - recent non-cover sales orders - other current commitments
...
They are drawn down through actual invoiced sales and therefore do not add to forecast
demand or reserve the projected pool a second time.  Non-cover sales orders older than
three calendar months are reported and ignored.
"""
...
def _commitment_position(
    ...
    *,
    ...
    explained_committed = recent_non_cover + stale_non_cover_ignored + current_cover
    reconciliation_delta = inventory.committed - explained_committed
    # Positive residual commitments remain reserved.  Negative deltas are surfaced but
    # never converted into extra stock.
    other_current = max(_ZERO, reconciliation_delta)
    physical_pool = inventory.on_hand - recent_non_cover - other_current
    # Cover orders are a separate coverage/alignment check.  They do not consume the
    # planning pool because actual invoiced sales are the draw-down signal.  Likewise,
    # supplier/YU On Order is inbound supply, not demand.
    ...
    *,
    ...
    if inventory is None or commitments is None:
        ...
        if commitments.uncovered_cover > 0:
            reasons.append(
                f"Cover alignment warning: customer cover exceeds YU/on-order by "
                f"{commitments.uncovered_cover}. This does not increase forecast demand "
                "or suggested replenishment."
            )
        if suggested_order > 0:
            reasons.append(
                f"Projected pool {commitments.projected_pool} is below target stock "
                f"{target_stock:.2f}."
            )
        ...
        if abs(commitments.committed_reconciliation_delta) > Decimal("0.000001"):
            reasons.append(
                "MYOB Committed differs from current cover, recent non-cover and stale "
                f"non-cover orders by {commitments.committed_reconciliation_delta}."
            )
        ...
        if commitments.immediate_shortage > 0:
            ...
            status = "order"
        elif commitments.uncovered_cover > 0 or (trend.significant and trend.delta > 0):
            status = "watch"
        else:
            status = "ok"
            reasons.append("Projected pool meets the current lead-time target.")
...
def get_order_analysis(
    ...
    *,
    ...
    for item, inventory_line in inventory_rows:
        ...
        if commitments.uncovered_cover > 0:
            reason_parts.append(
                f"cover alignment gap {commitments.uncovered_cover} (informational)"
            )
        if adjusted > 0:
            reason_parts.append(
                f"pool {commitments.projected_pool} is below target {target:.2f}"
            )
        if stale_non_cover != 0:
            reason_parts.append(f"ignored stale non-cover {stale_non_cover}")
        if abs(commitments.committed_reconciliation_delta) > Decimal("0.000001"):
            reason_parts.append(
                f"committed reconciliation {commitments.committed_reconciliation_delta}"
            )
        if trend.significant and trend.delta > 0:
            reason_parts.append(f"{trend_mode} invoiced demand increased by {trend.delta}")
        if commitments.immediate_shortage > 0:
            ...
            status = "ok"
        if not reason_parts:
            reason_parts.append("projected pool meets the sales-demand target")
