...
def test_trend_uses_recent_run_rate_without_double_counting_baseline() -> None:
    with Session(_engine()) as session:
        _seed(session)
        analysis = get_item_planning_analysis(
            session,
        ...
        assert analysis.trend.current_average == Decimal("20")
        assert analysis.trend.forecast_average == Decimal("20")
        # The old calculation used 15 + (20 - 10) = 25/month.
        assert analysis.trend.forecast_average != Decimal("25")
        expected_target = Decimal("20") * Decimal("98") / Decimal("30.4375")
        assert analysis.adjusted_target_stock == expected_target
